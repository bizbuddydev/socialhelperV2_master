...
def update_post_in_bigquery(page_id, previous_caption, updated_post):
    ...
    SET post_summary = @post_summary,
        ...
        themes = @themes,
        tone = @tone,
        date = CAST(CURRENT_DATE() AS STRING)
...
def main():
    ...
    with st.expander("Add an AI generated Post"):
        ...
        user_context = st.text_area("Optional: Add context for this post idea (e.g., seasonal theme, specific campaign focus, etc.)", placeholder = "e.g. I wanna post using this trend... I wanna promote a new product/service... ect.")
        ...
        if st.button("Add AI Generated Post", key="generate_post_id"):
            with st.spinner("Generating and adding post..."):
                ...
                strategy = {
                    "content_plan": [
                        "Fuel Vault should focus on a mix of engaging, informative, and promotional content to attract and convert followers into clients. Their Reels should include quick, actionable tips on improving athletic performance (e.g., mobility drills, strength training hacks, recovery techniques) and educational breakdowns of common training mistakes with clear fixes. To build trust, they should showcase client success stories, before-and-after progress clips, or testimonials from athletes who’ve used their training. Behind-the-scenes footage of workouts, coaching sessions, or day-in-the-life content can add authenticity and keep engagement high. Lastly, strategic promotional posts (such as limited-time offers, training program breakdowns, or Q&A sessions on their services) should be mixed in to convert viewers into paying clients."
                    ],
